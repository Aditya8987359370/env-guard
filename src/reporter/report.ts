...
const level: Record<Finding['severity'], string> = { critical: 'CRITICAL', high: 'HIGH', medium: 'MEDIUM', low: 'LOW', info: 'INFO' };
export function terminalReport(result: ScanResult): string {
  if (result.findings.length === 0) return `EnvGuard scan passed: ${result.scannedFiles} file(s) scanned.`;
  const rows = result.findings.map((f) => `${f.file}:${f.line}\n  ${f.type} [${level[f.severity]}]\n  Value: ${f.maskedValue}`);
